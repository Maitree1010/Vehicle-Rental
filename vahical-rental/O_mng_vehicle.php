						...
						<div class="table-wrapper">
							...
							<table class="table table-striped table-hover">
								...
								<tbody>
									...
									<?php
									if($search){
										$vselect="Select * from vehicles where owner_id=$oid AND (address like '%$search%' OR category like '%$search%' OR type like '%$search%' OR transmition like '%$search%' OR model like '%$search%' OR modelname like '%$search%' OR price like '%$search%' OR color like '%$search%' )";

									}else{
										$vselect="Select * from vehicles where owner_id=$oid";
									}
									...
									while ($vrow=mysqli_fetch_array($vqry)) 
									{
										...
										<?php
											...
											else if($status=='Unapprove')
											{
												...
												echo '<th><p class="bg-warning status" data-toggle="tooltip" title="In Process">In Process</p></th>';											

											}
										?>
										
										<th>
											<a href="O_vh_view.php?vid=<?= $vehicle_id;?>" class="edit ">
												<i class="material-icons " data-toggle="tooltip"
													title="Edit">&#xE254;</i>
											</a>
											<a href="#deleteEmployeeModal" onclick="sessionStorage.setItem('vid','<?= $vehicle_id; ?>');  go();" class="delete" data-toggle="modal">
												<i class="material-icons" data-toggle="tooltip"
													title="Delete">&#xE872;</i>
											</a>
										</th>
